...
public record OperatorInfo(
    ...
    private static final List<OperatorInfo> operators = List.of(
        ...
        info(OperatorID.MULT, OperatorType.BINARY, Multiplication.operator().notation()),
        info(OperatorID.DIV, OperatorType.BINARY, Division.operator().notation()),
        info(OperatorID.LN, OperatorType.UNARY, Logarithm.operator().notation()),
        info(OperatorID.SQ, OperatorType.UNARY, Squared.operator().notation()),
        info(OperatorID.POW, OperatorType.BINARY, Power.operator().notation()),
        info(OperatorID.SQRT, OperatorType.UNARY, SquareRoot.operator().notation()),
        info(OperatorID.CUBE_RT, OperatorType.UNARY, CubeRoot.operator().notation()),
